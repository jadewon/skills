...
스크립트가 hiworks 공휴일 체크(공휴일이면 "공휴일 스킵" 출력 후 게시 없이 0 종료) → CNN API 조회 → 포맷(점수/등급/게이지/비교표) → Slack webhook 게시까지 처리한다. 0 종료면 정상(게시 또는 공휴일 스킵), 비-0 종료면 stderr 의 에러를 보고하고 종료.
