...
Secrets: this repo is public, so Slack-posting skills load their webhook / bot / user token from a gitignored `.env` (see each skill's `.env.example`) — never commit real secrets. The `slack-*` skills that need a Slack **user** OAuth token (`xoxp-...`, everything except the `*-daily` webhook posters and `slack-scheduled-message`) all check a shared `~/.config/slack-user-token/.env` first, so you only paste the token once and just keep adding OAuth scopes to the same Slack App as you use more of them. `slack-scheduled-message` is the exception: it holds no token of its own and calls the local [slack MCP server](https://github.com/jadewon/mcps/tree/main/slack-mcp), which reads that same shared file.
